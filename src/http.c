...
http_nodogsplash_serve_splash(request *r, t_auth_target *authtarget, char *error_msg)
{
	...
	fd = popen("/usr/bin/aps", "r");
	if (fd == NULL) {
		httpdOutput(r, "Failed to run command\n" );
		exit(1);
	}
	  
	// Read the output a line at a time - output it. 
	while (fgets(line, MAX_BUF, fd) != NULL) {
		if (line_counter > 5 && strlen(line) > 16){
			pch = strtok(line," ");
			token_counter = 0;
			while (pch != NULL){
				strcpy(network_buffer[token_counter], pch);
				pch = strtok(NULL, " ");
				token_counter++;
			}
			if (token_counter == 8){
				sprintf(network_name, "<option value=\"%s\">%s - %s%%</option>", network_buffer[1], network_buffer[1], network_buffer[4]);
				strcat(networks, network_name);
			}
	...
	httpdAddVariable(r, "wifinetworks", networks);
